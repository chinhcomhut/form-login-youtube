...
public class SongAPI {
    ...
    @GetMapping("song-by-user")
    public ResponseEntity pageSongByUser(@PageableDefault(sort = "nameSong", direction = Sort.Direction.ASC)Pageable pageable){
        User user = userDetailsService.getCurrentUser();
        Page<Song> songPage = songService.findAllByUserId(user.getId(),pageable);
        if(songPage.isEmpty()){
            return new ResponseEntity(HttpStatus.NO_CONTENT);
        }
        return new ResponseEntity(songPage, HttpStatus.OK);
    ...
    @PostMapping("/song")
    public ResponseEntity<?> createSong(@Valid @RequestBody Song song, Singer singer) {
    ...
    public ResponseEntity<?> UpdateSong(@Valid @RequestBody Song song, @PathVariable Long id) {
        ...
        if(song.getNameSong()==null||song.getNameSong()==""){
